      ...
      builder: (context, snapshot) {
        ...
        if (docs.isEmpty) {
          return Container(
            ...
            child: Center(child: Text('Sin marcas registradas', style: theme.textTheme.bodySmall)),
          );
        }
        return Column(children: docs.asMap().entries.map((entry) {
          ...
          return GestureDetector(
            onTap: () => showEditBrandSheet(context, entry.value),
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(color: AdminTheme.primaryColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)),
                    child: const Icon(Iconsax.building, color: AdminTheme.primaryColor, size: 16),
                  ),
                  const SizedBox(width: 12),
                  Expanded(child: Text(data['name'] ?? entry.value.id, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14))),
                  Icon(Iconsax.arrow_right_3, size: 18, color: theme.textTheme.bodySmall?.color),
                ]),
                if (models.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Wrap(spacing: 6, runSpacing: 6, children: models.map((m) {
                    return Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(color: theme.colorScheme.surface, borderRadius: BorderRadius.circular(8)),
                      child: Text(m, style: theme.textTheme.bodySmall?.copyWith(fontSize: 12)),
                    );
                  }).toList()),
                ],
              ]),
            ).animate().fadeIn(duration: 250.ms, delay: (160 + entry.key * 30).ms),
          );
        }).toList());
      },
    );
        ...
        return Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('Nueva Marca', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
              const SizedBox(height: 20),
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Nombre')),
              const SizedBox(height: 12),
              TextField(controller: orderCtrl, keyboardType: TextInputType.number, inputFormatters: [FilteringTextInputFormatter.digitsOnly], decoration: const InputDecoration(labelText: 'Orden')),
              const SizedBox(height: 12),
              ...
              Row(children: [
                Expanded(child: TextField(controller: modelCtrl, decoration: const InputDecoration(hintText: 'Agregar modelo'))),
              ...
              AdminButton(
                ...
                onPressed: () async {
                  ...
                  await firestore.collection('equipmentCatalog').add({'name': nameCtrl.text.trim(), 'models': models, 'order': int.tryParse(orderCtrl.text) ?? 0});
