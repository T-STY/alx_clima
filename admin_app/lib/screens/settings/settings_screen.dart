...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  @override
  void dispose() {
    _nc.dispose(); _pc.dispose(); _wc.dispose(); _ec.dispose(); _hc.dispose(); _twc.dispose();
    for (final c in _pricingCtrl.values) { c.dispose(); }
    super.dispose();
  }
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              Text('Ajustes', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w700))
                  .animate().fadeIn(duration: 400.ms),
              const SizedBox(height: 24),

              _section(context, 'Información de la Empresa', Iconsax.building, [
                ...
                AdminButton(text: 'Guardar Empresa', icon: Iconsax.tick_circle, onPressed: () async {
                  await _firestore.collection('company').doc('info').set({
                    'name': _nc.text.trim(), 'phone': _pc.text.trim(), 'whatsApp': _wc.text.trim(),
                    'email': _ec.text.trim(), 'businessHours': _hc.text.trim(), 'techWarranty': _twc.text.trim(),
                  });
                  if (mounted) ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: const Text('Información actualizada'), backgroundColor: AdminTheme.successColor));
                }),
              ]),
              ...
              _section(context, 'Recargos y Descuentos', Iconsax.discount_shape, [
                _field(_ctrl('sf', 0), 'Recargo segundo piso (0.3 = 30%)', Iconsax.building_4, num: true),
                _field(_ctrl('df', 0), 'Recargo compresor piso diferente', Iconsax.arrow_swap, num: true),
                _field(_ctrl('md', 0), 'Descuento multi-equipo (0.1 = 10%)', Iconsax.discount_shape, num: true),
                const SizedBox(height: 12),
                AdminButton(text: 'Guardar Precios', icon: Iconsax.tick_circle, onPressed: () async {
                  await _firestore.collection('config').doc('pricing').set({
                    'installOnly': { '12000': num.tryParse(_ctrl('io12000', 0).text) ?? 0, '18000': num.tryParse(_ctrl('io18000', 0).text) ?? 0,
                      '24000': num.tryParse(_ctrl('io24000', 0).text) ?? 0, '36000': num.tryParse(_ctrl('io36000', 0).text) ?? 0 },
                    'fullPackage': { '12000': num.tryParse(_ctrl('fp12000', 0).text) ?? 0, '18000': num.tryParse(_ctrl('fp18000', 0).text) ?? 0,
                      '24000': num.tryParse(_ctrl('fp24000', 0).text) ?? 0, '36000': num.tryParse(_ctrl('fp36000', 0).text) ?? 0 },
                  ...
                  if (mounted) ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: const Text('Precios actualizados'), backgroundColor: AdminTheme.successColor));
                }),
              ]),
  ...
  Widget _field(TextEditingController ctrl, String label, IconData icon, {bool num = false}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: TextField(controller: ctrl, keyboardType: num ? TextInputType.number : null,
  ...
  List<Widget> _priceFields(String pfx) {
    ...
    return List.generate(btus.length, (i) => Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: TextField(controller: _ctrl('$pfx${btus[i]}', 0), keyboardType: TextInputType.number,
          decoration: InputDecoration(labelText: labels[i], prefixIcon: const Icon(Iconsax.money, size: 20), prefixText: '\$ ')),
    ));
  }
}
